...
int vsprintf(char* buffer, const char* format, va_list vlist) {
    size_t sourceLength =
        strlen(format);     // Maximum length we are going to read from 'format'
    ...
    while (go == 0) {
        ...
        if (format[sourceIndex] == '%' && format[sourceIndex + 1] == 'd') {
            ...
            for (numberCounter = 0; numberCounter < digitsLen;
                 numberCounter++) {
                buffer[bufferIndex++] = numberDigits[numberCounter];
            }
        } else if (format[sourceIndex] == '%' &&
                   format[sourceIndex + 1] == 's') {
            sourceIndex += 2;
